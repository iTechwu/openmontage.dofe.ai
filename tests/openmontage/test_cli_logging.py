...
def test_cli_entry_enables_info_logging_by_default(capsys) -> None:
    ...
    Without explicit configuration the root logger's effective level is WARNING,
    which silently drops the wrong-merge replay records logged at INFO. The CLI
    must raise that to INFO (the default) so the records are observable.
    """
    ...
    ), "CLI must attach the OpenMontage stderr handler at startup"
