...
**Preferred premium default — Seedance 2.0.** When any premium gateway is configured (`FAL_KEY` → `seedance_video`, or HeyGen's Video Agent / Avatar Shots path), Seedance 2.0 is the preferred default for cinematic, trailer, and high-fidelity clip work. It supports single-pass synchronized audio, multi-shot generation, camera control, and quoted dialogue. Switch off it only when the user has a specific reason such as budget, provider preference, or stylistic fit. Read `seedance-provider` for execution facts and `seedance-prompting` for the provider-ready prompt contract; use `seedance-directing`, `seedance-continuity`, and `seedance-quality` as the production context requires.
